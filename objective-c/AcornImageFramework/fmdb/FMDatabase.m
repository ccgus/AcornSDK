...
@implementation PXFMDatabase

+ (id)databaseWithPath:(NSString*)aPath {
    return [[[PXFMDatabase alloc] initWithPath:aPath] autorelease];
}
...
- (PXFMStatement*) cachedStatementForQuery:(NSString*)query {
    return [cachedStatements objectForKey:query];
}

- (void) setCachedStatement:(PXFMStatement*)statement forQuery:(NSString*)query {
...
- (BOOL) goodConnection {
    ...
    PXFMResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void) compainAboutInUse {
    NSLog(@"The PXFMDatabase %@ is currently in use.", self);
    
    if (crashOnErrors) {
        NSAssert1(false, @"The PXFMDatabase %@ is currently in use.", self);
    }
}
...
- (id) executeQuery:(NSString *)sql arguments:(va_list)args {
    ...
    PXFMResultSet *rs = nil;
    
    int rc                  = 0x00;;
    sqlite3_stmt *pStmt     = 0x00;;
    PXFMStatement *statement  = 0x00;
...
//                        asm{ trap };
    ...
    if (!statement) {
        statement = [[PXFMStatement alloc] init];
    ...
    rs = [PXFMResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
//                        asm{ trap };
    ...
    if (shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[PXFMStatement alloc] init];
